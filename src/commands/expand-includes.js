...
export class XiIncludeCodeLensProvider {
  ...
  provideCodeLenses(document) {
    ...
    const referenceRegex =
      /<reference\s+[^>]*target\s*=\s*"(?<url>[^"]+)"[^>]*\/?>[\s\S]+?<\/reference>/g
...
export function registerExpandIncludesCommands(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'draftforge.peekXiInclude',
      async (url, sourceUriString, line = 0, character = 0) => {
        try {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'draftforge.expandXiInclude',
      async (url, documentUri, range) => {
        try {
          const res = await fetch(url)
          if (!res.ok) {
            throw new Error(`HTTP ${res.status}: ${res.statusText}`)
          ...
          const editor = vscode.window.visibleTextEditors.find(
            (ed) => ed.document.uri === documentUri
          )
          if (!editor) {
          ...
          await editor.edit((editBuilder) => {
            ...
            const replaced = normalizedLines
              .map((l, i) => {
                if (l === '') return ''
                if (i === 0) return l
                return lineIndent + l
   ...
   */
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.foldReference', async (url, documentUri, range) => {
      try {
        const editor = vscode.window.visibleTextEditors.find(
          (ed) => ed.document.uri === documentUri
        )
        ...
        if (!bibUrl && url.startsWith('https://datatracker.ietf.org/doc/html/')) {
          const match =
            /https:\/\/datatracker.ietf.org\/doc\/html\/(?<name>[a-z0-9-]+)(.xml)?/i.exec(url)
