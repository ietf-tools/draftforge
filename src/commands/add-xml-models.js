...
export function registerAddXmlModelsCommand(context) {
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.addXmlModels', async function () {
      ...
      try {
        ...
        const rfc7991bisContent = await fetch(
          'https://github.com/ietf-tools/RFCXML/raw/main/rfc7991bis.rnc'
        )
        await vscode.workspace.fs.writeFile(
          vscode.Uri.parse(posix.join(dirPath, 'rfc7991bis.rnc')),
          new Uint8Array(await rfc7991bisContent.arrayBuffer())
        )
        const svg12rfcContent = await fetch(
          'https://github.com/ietf-tools/RFCXML/raw/main/SVG-1.2-RFC.rnc'
        )
        await vscode.workspace.fs.writeFile(
          vscode.Uri.parse(posix.join(dirPath, 'SVG-1.2-RFC.rnc')),
          new Uint8Array(await svg12rfcContent.arrayBuffer())
        )
      } catch (err) {
      ...
      if (replaceOccured) {
        vscode.window.showInformationMessage(
          'Schemas downloaded and existing <?xml-model> updated successfully.'
        )
      } else {
        await editor.edit((editBuilder) => {
          editBuilder.insert(
        ...
        vscode.window.showInformationMessage(
          'Schemas downloaded and <?xml-model> added successfully.'
        )
      }
    })
  )
