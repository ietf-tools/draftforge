...
export function registerCheckArticlesCommand(context, diagnosticCollection, ignores) {
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.checkArticles', async function (clearFirst = true) {
      if (clearFirst) {
      ...
      try {
        ...
        const partARgx =
          /(?<!(?:[aA]ppendix|[cC]onnection|[lL]ink|[nN]ode|Operator)) (?!(?:[aA] (?:AAA|Europe|[oO]ne|U[A-Z]|U-label|[uU]biquitous|[uU]nicast|[uU]nicode|[uU]nidir|[uU]nif|[uU]nion|[uU]nique|[uU]nit|[uU]nivers|[uU]sable|[uU]sability|[uU]sage|[uU]se|[uU]tility)|a uCDN|A and))[aA] [aeiouAEIOU]/g
        const partARRgx =
          /(?!(?:[aA] (?:RADIUS|RECEIVE|RECOMMENDED|REFER|RELOAD|RST|REALM|RESERVATION|REQUEST|RESET|ROUTE|RPL)))[aA] R[A-Z]/g
        const partBRgx =
          / (?!(?:[aA]n (?:hour|honest|honor|Mtrace|x-coordinate|x coordinate|A[A-Z]|E[A-Z]|F[A-Z]|H[A-Z]|I[A-Z]|L[A-Z]|L[0-9][A-Z]|M[A-Z]|N[A-Z]|O[A-Z]|R[A-Z]|R[0-9]|S[A-Z]|X[A-Z]|X\.509|xTR)))[aA]n [b-df-hj-np-tv-zB-DF-HJ-NP-TV-Z]/g
        const partCRgx =
          /[aA]n (?:AAA|FEC|FIR|LIS|LRDD|MEG|MEP|MRHOF|MIC|NAPTR|NAT|NAS|RAS|ROHC|RPL|RST|SAFI|SCSI|SID|SIP|SMPTE|SYN|rinit)/g
        ...
        if (diags?.length > 0) {
          ...
          vscode.window.showInformationMessage(
            'No incorrect article usaged detected in this document.'
          )
        }
      } catch (err) {
        console.warn(err)
