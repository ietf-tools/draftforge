...
export function registerIdnitsCommand(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.idnits', async function (mode = 'normal') {
      try {
            ...
            try {
              ...
              for (const valGroup of validations) {
                ...
                for (const valTask of valGroup.tasks) {
                  ...
                  try {
                    ...
                    if (!valTask.isVoid && Array.isArray(taskResults)) {
                      ...
                      for (const nit of taskResults) {
                        ...
                        results[taskIdx].nits.push(
                          Object.assign({ message: nit.message, result }, nit)
                        )
                      }
                    }
                  ...
                  results[taskIdx].perf = +performance
                    .measure(valTask.key, `${valTask.key}_start`, `${valTask.key}_end`)
                    .duration.toFixed(2)
                  taskIdx++
                }
                performance.mark(`${valGroup.key}_end`)
                resultGroups[grpIdx].perf = +performance
                  .measure(valGroup.key, `${valGroup.key}_start`, `${valGroup.key}_end`)
                  .duration.toFixed(2)
