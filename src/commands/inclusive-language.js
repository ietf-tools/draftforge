...
export function registerCheckInclusiveLanguageCommand(context, diagnosticCollection, ignores) {
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'draftforge.checkInclusiveLanguage',
      async function (clearFirst = true) {
        ...
        try {
          ...
          const matchRgx = new RegExp(
            `(?:^|[<> "'.:;=([{-])(?<term>${flatten(dictionnary.map((d) => d.triggers)).join('|')})(?:[^a-z0-9]|$)`,
            'gi'
          )
          ...
          for (let lineIdx = 0; lineIdx < activeDoc.lineCount; lineIdx++) {
            ...
            for (const match of line.text.matchAll(matchRgx)) {
              ...
              const diag = new vscode.Diagnostic(
                new vscode.Range(
                  lineIdx,
                  match.index + termStartIndex,
                  lineIdx,
                  match.index + termStartIndex + match.groups.term.length
                ),
                dictEntry.suggestion
                  ? `Inclusive Language: Consider using ${dictEntry.suggestion} instead of "${term}".`
                  : `Inclusive Language: "${term}" is potentially biased or ambiguous.`,
