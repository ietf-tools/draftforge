...
export function registerCheckTyposCommand(context, diagnosticCollection, ignores) {
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.checkTypos', async function (clearFirst = true) {
      if (clearFirst) {
      ...
      try {
        ...
        const matchRgxCI = new RegExp(
          `(?:^|[<> "'.:;=([{-])(${flatten(dictionnary.filter((d) => !d.caseSensitive).map((d) => d.triggers)).join('|')})(?:[^a-z0-9]|$)`,
          'gi'
        )
        const matchRgxCS = new RegExp(
          `(?:^|[<> "'.:;=([{-])(${flatten(dictionnary.filter((d) => d.caseSensitive).map((d) => d.triggers)).join('|')})(?:[^a-zA-Z0-9]|$)`,
          'g'
        )
        ...
        const matchRgxBCP14 =
          /(MUST not|SHOULD not|SHALL not|MAY not|MAY NOT|not RECOMMENDED|OPTIONALLY|RECOMMEND|RECOMMENDS|RECOMMENDING)(?:[^a-zA-Z0-9]|$)/g
        ...
        function processMatch(term, match, lineIdx, caseSensitive = false) {
          const dictEntry = dictionnary.find((d) => d.triggers.includes(term))
          ...
          addDiag(
            new vscode.Range(
              ...
              startColumnAdjusted,
              lineIdx,
              startColumnAdjusted + match[1].length
            ),
            caseSensitive
              ? `Possible typo: ${term}. Did you mean "${dictEntry.suggestion}"? (case sensitive)`
              : `Possible typo: ${term}. Did you mean "${dictEntry.suggestion}"?`,
            term
          )
        ...
        for (let lineIdx = 0; lineIdx < activeDoc.lineCount; lineIdx++) {
          ...
          const combinedLine =
            lineIdx + 1 < activeDoc.lineCount
              ? `${lineTrimmed} ${activeDoc.lineAt(lineIdx + 1).text.trimStart()}`
              : line.text
