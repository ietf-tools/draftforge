...
export function registerCheckRfcTermsCommand(context, diagnosticCollection, ignores) {
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.checkRfcTerms', async function (clearFirst = true) {
      if (clearFirst) {
      ...
      try {
        ...
        const matchRgx =
          /(?:^|[<> "'.:;=([{-])(?<term>RFC series|IETF stream|IAB stream|IRTF stream|independent stream|IAB-stream|IRTF-stream|IETF-stream|internet draft|last call|chair|director|IETF member|IAB member|IETF engineer|earlier version|previous version|future version|IETF RFC|IAB RFC|IRTF RFC|standards track|standards-track|experimental|informational|best current practice|historic|proposed standard|draft standard|internet standard|full standard|working group|area director|shepherd)(?:[^a-z0-9]|$)/gi
          ...
          for (const match of line.text.matchAll(matchRgx)) {
            ...
            const diag = new vscode.Diagnostic(
              new vscode.Range(
                ...
                startColumnAdjusted,
                lineIdx,
                startColumnAdjusted + term.length
              ),
