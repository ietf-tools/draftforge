...
const AUTHOR_NAMES_URL =
  'https://github.com/rfc-editor-drafts/common/raw/refs/heads/main/names.json'
...
export function registerCheckNamesCommand(context, diagnosticCollection, ignores) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.checkNames', async function (clearFirst = true) {
      if (clearFirst) {
      ...
      try {
        ...
        const matchRgx = new RegExp(
          `(?:^|[<> "'.:;=([{-])(?<term>${flatten(names.map((d) => d.match)).join('|')})(?:[^a-z0-9]|$)`,
          'gi'
        )
        ...
        for (let lineIdx = 0; lineIdx < activeDoc.lineCount; lineIdx++) {
          ...
          for (const match of line.text.matchAll(matchRgx)) {
            ...
            const dictEntry = names.find((d) => d.match.includes(term))
