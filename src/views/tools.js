...
class ToolsProvider {
  ...
  populateTools() {
    ...
    this.tools = [
      flags.xml && {
        id: 'addXmlModels',
        label: 'Add XML Models',
        description: 'Download and add RelaxNG schema files to the document directory',
        icon: 'cloud-download'
      },
      {
        id: 'exportAs',
        label: 'Export as...',
        description: 'Generate multiple outputs of the current document',
        icon: 'files'
      },
      {
        id: 'exportHtml',
        label: 'Export as HTML',
        description: 'Generate HTML output of the current document',
      ...
      },
      {
        id: 'exportTxt',
        label: 'Export as TXT',
        description: 'Generate TXT output of the current document',
        icon: 'file-text'
      },
      flags.rpc && {
        id: 'extractComments',
        label: 'Extract [rfced] Comments',
        description: 'List all comments for the RPC staff',
        icon: 'comment'
      },
      {
        id: 'extractCodeComponents',
        label: 'Extract Code Components',
        description: 'Extract all or some sourcecode blocks',
        icon: 'file-code'
      },
      {
        id: 'formatDocument',
        label: 'Format Document',
        description: 'Reformat document and fix indentation',
        icon: 'list-flat'
      },
      {
        id: 'idnits',
        label: 'IDNits',
        description: 'Run idnits on the current document',
        icon: 'tasklist'
      },
      {
        id: 'abbreviations',
        label: 'List Abbreviations',
        description: 'List abbreviation expansion usage',
        icon: 'whole-word'
      },
      {
        id: 'inconsistentCapitalization',
        label: 'List Inconsistent Capitalization',
        description: 'List inconsistent use of capitalization',
        icon: 'case-sensitive'
      },
      {
        id: 'inconsistentFormatting',
        label: 'List Inconsistent Formatting',
        description: 'List inconsistent formatting like bold, italics, etc.',
        icon: 'paintcan'
      },
      {
        id: 'lookupSelectionAcrossDocs',
        label: 'Lookup Selection Across Docs',
      ...
      // },
      {
        id: 'openPreview',
        label: 'Open Preview',
        description: 'Open a preview of the current document',
        icon: 'open-preview'
      },
      flags.rpc && {
        id: 'prepareForPublishing',
        label: 'Prepare for Publishing',
        description: 'Ensure repository is ready for publishing',
        icon: 'mortar-board'
      },
      {
        id: 'stripMLineEndings',
        label: 'Strip ^M Line Endings',
        description: 'Clean Document from ^M Line Endings',
        icon: 'no-newline'
      },
      {
        id: 'surroundBcp14Keywords',
        label: 'Surround BCP 14 Keywords',
        description: 'Ensure all BCP 14 keywords are enclosed with <bcp14> tags',
        icon: 'bold'
      },
      flags.xml && {
        id: 'svgcheck',
        label: 'SVG Check',
        description: 'Validate SVGs in the current document',
        icon: 'circuit-board'
      }
    ].filter((t) => t)
  }
...
export function activateToolsView(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.runTool', async (tool) => {
      try {
        ...
        switch (tool.id) {
          ...
          case 'makeDiff': {
            await vscode.commands.executeCommand('draftforge.makeDiff')
            break
          }
          case 'openPreview': {
            if (doc.languageId === 'xml') {
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor(() => toolsProvider.refresh())
  )
}
