  ...
  constructor() {
    ...
    this.checks = [
      new CheckItem(
        'articles',
        'Articles Check',
        'Check for bad indefinite articles usage',
        'repo'
      ),
      new CheckItem(
        'hyphenation',
        'Hyphenation Check',
        'Check for inconsistent hyphenation usage',
        'diff-removed'
      ),
      new CheckItem(
        'inclusiveLanguage',
        'Inclusive Language Check',
        'Check for usage of non-inclusive terms',
        'heart'
      ),
      ...
      new CheckItem(
        ...
        'Check for RFC-specific terms usage',
        'coffee'
      ),
      new CheckItem(
        'repeatedWords',
        'Repeated Words Check',
        'Check for accidental repeated terms',
        'layers'
      ),
...
export async function activateChecksView(context, diagnosticCollection) {
  ...
  const checksView = vscode.window.createTreeView('draftforge-checks', {
    treeDataProvider: checksProvider
  })
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.runCheck', async (check, clearFirst = true) => {
      try {
  ...
  // Run All Checks
  context.subscriptions.push(
    vscode.commands.registerCommand('draftforge.runAllChecks', async () => {
      try {
        const activeDoc = vscode.window.activeTextEditor?.document
        if (!activeDoc) {
        ...
        if (diags?.length > 0) {
          for (const diag of diags) {
            ...
            if (diag.match && diag.range.contains(range)) {
              // Repo scope
              const actRepo = new vscode.CodeAction(
              ...
              // Document scope
              const actDoc = new vscode.CodeAction(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      'draftforge.ignoreWarning',
      async (scope = 'repo', documentUri, diag) => {
        try {
          const workspacePath = vscode.workspace.getWorkspaceFolder(documentUri).uri.fsPath

          // Add to editor ignores
          const currentIgnoreValues = ignores[documentUri.toString()][diag.code] ?? []
          currentIgnoreValues.push(diag.match)
          ignores[documentUri.toString()][diag.code] = uniq(currentIgnoreValues)
          ...
          const manifestIgnores = manifest[ignorePath] ?? []
          manifestIgnores.push(diag.match)
          void manifestManager.updateManifest(
            workspacePath,
            ignorePath,
            uniq(manifestIgnores),
            true
          )
  ...
  context.subscriptions.push(
    vscode.workspace.onDidOpenTextDocument(async (doc) => {
      if (doc.uri.scheme === 'file') {
        void loadIgnoresForDocument(doc.uri)
      }
    })
  )
...
async function loadIgnoresForDocument(docUri) {
  ...
  const manifest = await manifestManager.getManifest(workspacePath)
  ignores[docUri.toString()] = manifest?.draftforge?.ignores?.global ?? {}
  ignores[docUri.toString()] = merge(
    ignores[docUri.toString()],
    manifest?.draftforge?.ignores?.[documentPathHash] ?? {}
  )
}
